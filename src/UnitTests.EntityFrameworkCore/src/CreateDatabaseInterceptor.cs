    ...
    where TDbContext : DbContext
{
    ...
    {
        if (dbCreating || dbCreated || dbContext == null)
            return;

        // Avoid stack overflow in scenarios where createDb() calls eg. dbContext.Database.EnsureCreated
        dbCreating = true;
        try
        {
